...
// Super Cyphs 64: turn common enemies into neutral "Foe-id" placeholder models.
// This changes their visual model only; their original behavior/hitboxes remain intact.
#define SUPER_CYPHS_ENABLE_ENEMY_FOEIDS 0
#define SUPER_CYPHS_FOEID_MODEL MODEL_LUIGI

static s32 super_cyphs_is_enemy_behavior(const BehaviorScript *behavior) {
#define CYPHS_BHV_IS(name) (behavior == segmented_to_virtual(name))
    return CYPHS_BHV_IS(bhvGoomba)
        || CYPHS_BHV_IS(bhvBobomb)
        || CYPHS_BHV_IS(bhvKoopa)
        || CYPHS_BHV_IS(bhvFlyGuy)
        || CYPHS_BHV_IS(bhvChuckya)
        || CYPHS_BHV_IS(bhvThwomp)
        || CYPHS_BHV_IS(bhvThwomp2)
        || CYPHS_BHV_IS(bhvPiranhaPlant)
        || CYPHS_BHV_IS(bhvBoo)
        || CYPHS_BHV_IS(bhvBooWithCage)
        || CYPHS_BHV_IS(bhvBooInCastle)
        || CYPHS_BHV_IS(bhvChainChomp)
        || CYPHS_BHV_IS(bhvEnemyLakitu)
        || CYPHS_BHV_IS(bhvSpiny)
        || CYPHS_BHV_IS(bhvBulletBill)
        || CYPHS_BHV_IS(bhvMrI)
        || CYPHS_BHV_IS(bhvSwoop)
        || CYPHS_BHV_IS(bhvScuttlebug)
        || CYPHS_BHV_IS(bhvSnufit)
        || CYPHS_BHV_IS(bhvSkeeter)
        || CYPHS_BHV_IS(bhvUkiki)
        || CYPHS_BHV_IS(bhvMoneybag)
        || CYPHS_BHV_IS(bhvKlepto)
        || CYPHS_BHV_IS(bhvPokey)
        || CYPHS_BHV_IS(bhvMadPiano);
#undef CYPHS_BHV_IS
}

static u32 super_cyphs_model_for_behavior(u32 model, const BehaviorScript *behavior) {
#if SUPER_CYPHS_ENABLE_ENEMY_FOEIDS
    if (super_cyphs_is_enemy_behavior(behavior)) {
        return SUPER_CYPHS_FOEID_MODEL;
    }
#endif
    return model;
}
...
struct Object *spawn_object_at_origin(struct Object *parent, UNUSED s32 unusedArg, u32 model,
    ...
    model = super_cyphs_model_for_behavior(model, behaviorAddr);
